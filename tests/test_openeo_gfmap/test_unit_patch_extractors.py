...
@pytest.mark.parametrize("epsg, expected_coords", [
    (4326, {LAT_HARMONIZED_NAME, LON_HARMONIZED_NAME}),
    (3757, {LAT_HARMONIZED_NAME, LON_HARMONIZED_NAME})
])
def test_get_latlons_epsg(mock_feature_extractor, mock_data_array, epsg, expected_coords):
    with patch('pyproj.Transformer.from_crs', return_value=create_mock_transformer()) as mock_from_crs:
...
# test excecute
@patch.object(DummyPatchFeatureExtractor, '_common_preparations', return_value=create_mock_common_preparations())
@patch.object(DummyPatchFeatureExtractor, '_rescale_s1_backscatter', return_value=create_mock_rescale_s1())
