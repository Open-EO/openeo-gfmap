...
For a <s>training workflow</s> the Data Fetching, Preprocessing, Features and Job Manager sections are worth a read.

For a <s>inference workflow</s> all the above as well as the Inference features are interesting.

![OpenEO GFMAP Core Worklow](../worklow.png)

## Sections

This documentation covers the different building blocks that an user can use on OpenEO GFMAP:

* [Data Fetching](#fetching): Allows to fetch multiple data sources and performs source specific preprocessing (backscatter computation for Sentinel1, resolution resampling, ...).
* Preprocessing: Offers preprocessing functionalities common to all data sources.
